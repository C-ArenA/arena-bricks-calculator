...
export const useCalculatorStore = defineStore('calculator', () => {
  ...
  const mortarPerBlockArea = computed(() => mortar.value.width * (brick.value.length + brick.value.height + mortar.value.width))
  const brickArea = computed(() => brick.value.length * brick.value.height)
  const mortarBrickRatio = computed(() => mortarPerBlockArea.value / (brickArea.value + mortarPerBlockArea.value))
  ...
  const cementNeededVolume = computed(() => mortarDryVolume.value * mortar.value.cementSandRatio[0] / (mortar.value.cementSandRatio[0] + mortar.value.cementSandRatio[1]))
  const cementDensity = computed(() => mortar.value.cement.bagWeight / mortar.value.cement.bagVolume)
  const cementNeededWeight = computed(() => cementNeededVolume.value * cementDensity.value)
  const cementNeededBags = computed(() => Math.ceil(cementNeededWeight.value / mortar.value.cement.bagWeight))
  const sandNeededVolume = computed(() => cementNeededVolume.value * mortar.value.cementSandRatio[1])
  const waterNeeded = computed(() => cementNeededWeight.value * mortar.value.waterCementWeightRatio)
  return { walls, mortar, brick, wallsArea, totalBricksNeeded, cementNeededBags, sandNeededVolume, waterNeeded, mortarDryVolume, mortarVolume}
})
